...
namespace DataUtilities.Attributes
{
    ...
    public class ExcelTemporaryStorageAttribute: Attribute
    {
        ...
        public static string GetExcelTemporaryStorageColumn(Type type, string propertyName)
        {
            PropertyInfo[] properties = type.GetProperties();
            foreach (PropertyInfo property in properties)
            {
                if (property.Name.Trim().ToLower().Equals(propertyName.Trim().ToLower()))
                {
                    ...
                    if (attributes.Length > 0)
                    {
                        string value = ((ExcelTemporaryStorageAttribute)attributes[0]).Name.ToString();
                        return value;
                    }
                }
            }
